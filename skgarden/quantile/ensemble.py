...
from skgarden.forest import ForestRegressor
...
# note designed to be initialised!
class BaseForestQuantileRegressor(ForestRegressor):
    ...
    def fit_adj(self, X, y):
        """
        Build a forest from the training set (X, y). Adjusted according to the
        solution provided by fennovj: https://github.com/scikit-garden/scikit-garden/issues/74
        ...
        for i, est in enumerate(self.estimators_):
            if self.bootstrap:
                bootstrap_indices = generate_sample_indices(
                    est.random_state, len(y))
                # random_state ensures that splits are reproducible resp. controlled
            else:
                bootstrap_indices = np.arange(len(y))

            est_weights = np.bincount(bootstrap_indices, minlength=len(y))
            # np.bincount -> counts the number of occurrences of each value in an array of
            #  non-negative integers -> counts how many times a index is within bootstrap_indices
            y_train_leaves = est.y_train_leaves_
            for curr_leaf in np.unique(y_train_leaves):
                y_ind = y_train_leaves == curr_leaf
                self.y_weights_[i, y_ind] = (
    ...
    def predict_approx(model, X_test, quantiles=[0.5]):
        """
        ...
        source: https://github.com/scikit-garden/scikit-garden/issues/74
        """
